...
命令会生成长边 `800px`、`1280px` 与 `1920px` 的 sRGB 渐进式 JPEG，删除 EXIF 元数据。Markdown 只引用不带尺寸后缀的 `1920px` 主文件；构建时自动加入响应式 `srcset`、固有宽高、首图优先加载和其余图片懒加载。图集文章的 Front Matter 设为 `lazyload: false`，避免主题旧式懒加载与响应式图片冲突。

发布前确认：

- 文件名可读且稳定，不使用临时截图名称；
- 图片方向正确，无无意中包含的定位或隐私信息；
- 照片已按实际展示尺寸生成响应式版本，主文件原则上不超过约 `1.5MB`；
- 每张有意义的图片都有替代文字；
