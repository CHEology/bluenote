...
- 独立 Gallery 的图片区最大为 `1080px`；《秋之纽约_2023.11》作为含多张照片的普通博客特例，可以保留同样宽度的图片区，导语仍回到普通 `760px` 内容列。
...
- Gallery 只收录作者明确交付的照片，严格按清单顺序排列；不得自行补写图注、分组名或叙事说明。
- 照片尚未交付时，只显示“尚未收录照片。”，不放样片、占位图、加载骨架、虚构照片数量或自拟介绍。

### 8.3 Gallery 排列与观看

- 页面上方只显示 Gallery 标题，不另加大幅封面或副标题。图片区最大宽度为 1080px，并与页眉标题对齐。
- 桌面使用按原比例齐行的布局：每行通常两张，竖幅组合可三张；宽高比至少 2.7 的宽幅照片单独成行。分行只能改变容器组合，不改变作者给定顺序。
- 同一行照片等高，宽度依原比例分配；不裁切、不拉伸、不加新的画框。末行单张普通照片不强行铺满整行。
- 照片间距为 24–32px，手机一列；有原稿图注时在图下展示，字号 0.875rem、行距 1.6。无图注时不填充任何说明。
