...
- 文末导航采用左右等宽、中间自适应的三列布局；上一篇在左、下一篇在右，中间以向上角箭头返回页首。三向箭头共用主题 SVG 样式：`24 × 24px`、线宽 `2`、圆角端点与连接；向上箭头是左右角箭头的同形转向，不加箭杆、圆圈或常驻文字。悬停及辅助技术名称为 Back to top（中文界面为“回到页首”）。
- 中间入口始终居中，不因一侧缺少相邻文章而偏移；列间距 `0.75rem`，三处操作区域至少 `44px` 高，页首入口至少 `44px` 宽。中间箭头沿用两侧的 `--link-hover` 变色，在箭头下方显示 `24px × 1px` 细线，键盘焦点反馈相同；图标保持静止。
- 页首入口使用指向文章页眉的原生页内链接，立即回到页首并将焦点带回页眉，无 JavaScript 时仍可使用。不悬浮，不在正文中添加内容。
- 文末上一篇／下一篇的悬停与键盘焦点统一采用 `--link-hover` 变色和文字 `1px` 细下划线（偏移 `0.2em`）；键盘焦点另有清晰轮廓。左右箭头保持静止，不播放位移动画。
...
- 作者明确选择的文章通过 Front Matter 的 `companion` 链接到相关文字或 AI 回应。按最新确认，入口为正文与可选标签之后、上一篇／下一篇之前的狭长横框；关联文字入口保持独立，不占用上下篇导航中的位置。页眉和正文开头不添加入口。
...
- 作者明确选曲时使用已确认的 D 封面式播放器，置于正文前，与普通正文同宽；无底色、无外框、无阴影、不悬浮。以文章的 `music` 元数据选择启用，歌曲信息不写入正文。添加方法与资源路径见 [`PUBLISHING.md` §3.1](./PUBLISHING.md#31-文章配乐)。
- 横向排列专辑封面与歌曲信息、控制条。桌面封面 `100px`、间距 `20px`；手机（≤767px）封面 `76px`、间距 `14px`。封面保留完整比例，圆角 `2px`，不生成图注或接入图片灯箱。组件上下内边距 `0.5rem`，下方留 `2rem` 后进入原文。
- 沿用正文衬线字体。歌名桌面 `1rem`、手机 `0.9375rem`，艺人、专辑、时间及状态文字 `0.8125rem`，行距 `1.65`；歌名自然换行，不截断。文字及控件均使用既有 `--heading`、`--prose`、`--muted`、`--link-hover` 与 `--line`，明暗模式同时生效。
- 控制条提供播放／暂停、可拖动且可键盘调整的进度、已播／总时长。桌面另有静音和音量；手机沿用 D 版布局，音量交给设备系统控制。按钮及滑杆操作区域至少 `44px` 高，按钮宽 `44px`；进度轨道 `2px`，游标 `8px`，使用实际播放位置区分已播与未播段。
- 音频与封面全部自托管，不使用 Spotify 或第三方播放器。完整音频只在读者主动播放或调整进度时加载（`preload="none"`）；不自动播放、不循环、不自动记住播放进度或跨页续播。
- 控件有无障碍名称及可见键盘焦点。初始 HTML 与首屏样式直接呈现 D 版控件，脚本只绑定行为并启用操作，不在首屏绘制后替换原生播放条；脚本未就绪前操作暂不可用，但外观与尺寸不变。禁用 JavaScript 或播放器脚本加载失败时显示原生控件，仍能播放。加载失败显示提示和“打开音频”链接，再次点击播放可重试。不会因播放失败隐藏原文。
- 初次进入、刷新、播放、暂停、恢复与拖动缓冲过程不得改变播放器、封面、控制条或正文的位置。艺人／专辑信息始终可见，不用加载文字替换或遮住。等待持续超过 400ms 时，才在播放按钮的固定区域内显示细环，并向辅助技术报告加载状态；短暂等待不显示细环，减少动态效果时细环不旋转。只有实际失败时，才在控制条下显示错误与恢复链接。
- 样式和播放行为由站点统一组件维护，仅选曲文章加载资源；预览中的 A／B／C 不作为线上变体保留。
...
- Gallery 默认打开 `/gallery/` 的 **A few** 模式，每次进入或主动点击 **Shuffle**，从已收录作品中随机展示 3–5 张；`/gallery/all/` 的 **All photographs** 保留 8.3 节的完整编排。主导航仍只有一个 Gallery 入口。
- 两种模式用页面内的衬线文字导航切换，当前模式有细下划线。Shuffle 保留上下两个同功能入口：顶部入口位于模式切换同一行右侧，便于立即更换首轮选片；底部入口在最后一个展位之后居中，便于看完后继续。两处均使用小型回转符号与文字、无填充按钮，不使用卡片、粗边框、彩色主按钮或悬浮固定控件；字号 `0.9375rem`，触控高度至少 `44px`，窄屏自然换行。
...
- 每次 Shuffle 生成的每个单幅或双联都必须重新放入 8.3 节规定的独立展位；随机组合只改变本次选片和作品之间的并置关系，不得压缩展位或让相邻组侵入当前作品的标准观看画面。
- 底部 Shuffle 成功生成新一轮后，将焦点移至第一张作品并把它带回视窗中央；常规情况下使用平滑滚动，系统请求减少动态效果时立即定位。顶部入口不改变当前滚动位置。
...
- 桌面与平板首页卡片保持方形、无圆角、低阴影；一张卡只表达标题、摘要和必要元信息。手机（≤767px）采用已确认的 A 方案：日期、标题、短摘要组成无方框的单列文章目录。
- 首页启用主题 `home.layout: split`，最多展示 12 张卡片。桌面（≥992px）以视口中央为轴，左右各容纳两列、每列三张，中间保留 `clamp(8rem, 16vw, 16rem)` 加两道卡片间距的连续留白，让封面人物及倒影保持可见。卡片尺寸同时服从视口高度和两侧可用宽度，不能侵入中央留白。
- 桌面分区布局中，卡片的横向、纵向间距统一为 `clamp(1.25rem, 2vw, 1.5rem)`（20–24px，通常 24px）。卡片区上下留白相等，至少 `clamp(1.5rem, 4vh, 3rem)`（24–48px），不再为已滚走的导航额外预留顶部空间；整个 12 位网格水平居中、三行在卡片区垂直居中，左右两区等宽，空置右区也保留位置。卡片继续按可用高度和宽度适配，保持正方形；平板沿用原有间距与布局，手机采用下述文章目录。
- 卡片按日期顺序先填满左侧两列六个位置，再从第 7 张起填充右侧两列六个位置；每列自上而下、列与列之间从左向右排列。不足 7 张时右侧留空，空位随文章增多自然填充，不因数量变化而均分左右或移动既有列位置。768–991px 恢复三列四行，≤767px 为单列目录，均保留日期顺序，不强行保留桌面中央通道。
- 手机目录取消卡片等宽高比例、独立填充底色与阴影，两侧留白 `24px`，列表上方 `48px`、下方 `64px`；每项上 `25px`、下 `27px`，以低对比度 `1px` 细线分隔。日期置于标题上方，`12px`、行距 `1.5`；标题 `21px`、字重 `500`、行距 `1.5`，完整自然换行；摘要 `15px`、行距 `1.75`，最多两行，沿用现有文章摘要，不改写正文。日期下留 `9px`、标题下留 `10px`。整项可点击，键盘焦点可见。
- 手机保留封面照片、标语与深蓝色体系；目录采用统一的半透明深蓝底色 `rgba(6,21,33,.94)` 保障文字可读，上沿直接衔接照片，不使用渐变、羽化或向外延伸的过渡层。桌面与平板封面及卡片布局不随本次手机目录调整。
- 导航以文字为主，不恢复装饰性分类图标。
- 全站导航只位于文档页首，首页、文章、Gallery、归档、搜索、About 等页面采用相同行为：向下滚动时随页眉自然移出视窗，回到页首时重新可见。不吸顶、不缩小、不随滚动变色，也不在稍微向上滚动时弹回。首页页首保留完整站名和菜单、既有底色及封面关系；内容页保留导航覆盖在页眉上的初始位置，不额外增加占位或改变正文起点。
- 手机菜单只有在读者主动展开时才固定为全屏面板，并锁定背景滚动；站名与关闭按钮置于面板之上。关闭后恢复导航随文档滚动的状态。
...
- 手机菜单关闭时不可获得焦点；搜索打开时背景不可操作，Tab 在弹窗内部循环，关闭后恢复触发入口的焦点。搜索须区分加载中、无结果与加载失败，并提供重试。首次打开且关键词为空时，只显示搜索输入栏，不因索引载入插入再删除临时加载行；载入前后面板及输入栏的位置、宽高保持不变。辅助技术通过输入框的忙碌状态获知加载；输入关键词后才在结果区显示加载提示、匹配结果或无结果提示，实际失败始终显示错误与重试入口。
...
- `scripts/post-music.js`：校验文章 `music` 元数据与本地资源、生成统一配乐标记，并按需注入样式和脚本；不改写作者正文。
...
2. 检查文章页桌面与手机宽度；
3. 检查浅色与深色模式；
4. 检查一段同时含中文、英文、数字、斜体和链接的正文；导航修改还须验证首页与文章页向下滚动后导航完全离开视窗、稍向上滚动不弹回、回到页首恢复，以及手机菜单的开关、搜索与焦点恢复；
