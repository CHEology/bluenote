...
- 控件有无障碍名称及可见键盘焦点。初始 HTML 保留原生音频控件，脚本初始化成功后才替换为 D 版；脚本不可用仍能播放。加载失败显示提示和“打开音频”链接，再次点击播放可重试。不会因播放失败隐藏原文。
- 播放、暂停、恢复与缓冲过程不得改变播放器、封面、控制条或正文的位置。加载提示与艺人／专辑信息共享同一行，保留原信息的换行占位；开始播放或暂停后恢复原信息，不插入或删去额外的加载行。已缓存、可立即播放的音频恢复时不显示加载提示。只有实际失败时，才在控制条下显示错误与恢复链接。
...
- 桌面分区布局中，卡片的横向、纵向间距统一为 `clamp(1.25rem, 2vw, 1.5rem)`（20–24px，通常 24px）。卡片区上下留白相等，至少 `clamp(1.5rem, 4vh, 3rem)`（24–48px），不再为已滚走的导航额外预留顶部空间；整个 12 位网格水平居中、三行在卡片区垂直居中，左右两区等宽，空置右区也保留位置。卡片继续按可用高度和宽度适配，保持正方形；平板与手机沿用原有间距与布局。
- 卡片按日期顺序先填满左侧两列六个位置，再从第 7 张起填充右侧两列六个位置；每列自上而下、列与列之间从左向右排列。当前 6 张全部位于左侧，右侧空位随文章增多自然填充，不因数量变化而均分左右或移动既有列位置。768–991px 恢复三列四行，≤767px 为单列，均保留日期顺序，不强行保留桌面中央通道。
- 导航以文字为主，不恢复装饰性分类图标。
