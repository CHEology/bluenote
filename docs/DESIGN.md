...
- 图集照片统一生成 `800px`、`1280px` 和 `1920px` 三档长边尺寸。浏览器通过 `srcset` 按屏幕选择，不让手机下载桌面大图。
- 首图优先加载，其余图片原生懒加载；所有图片写入固有宽高，避免载入时页面跳动。
- 发布文件删除定位、拍摄时间等 EXIF 元数据，并保留 sRGB 色彩描述。原始大图应留在作者自己的照片库或备份中，不直接提交到网站仓库。
- 单张 `1920px` 照片原则上控制在约 `1.5MB` 内；如果画面细节导致体积更大，应人工检查后再决定质量与尺寸，而不是粗暴继续压缩。

### 8.2 文章集与图集

- 文章集与图集是并列的内容入口，共享 Blue Note 的颜色、字体、页眉、归档规则与原文原则。
- 文章集以文字阅读为主，图片服从正文行宽；图集以观看顺序为主，使用最大 `1080px` 的图片区，导语仍使用普通文字列。
- 图集可以有独立索引页与导航入口，但单个图集仍使用稳定、可分享的永久链接，并可在总归档中出现。
- 图集的标题、导语、图片顺序和图注都属于原稿；排版不得自行补写图注、分组名或叙事说明。
- 图集索引只显示作者已经提供的标题、日期和封面信息。没有封面授权时，使用克制的纯文字条目，不任意裁切正文照片充当封面。
