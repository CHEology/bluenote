...
- 首页网格、无图编辑式内容页、文章阅读配色和宽版图集均已迁移到源码层。
- 明暗配色默认跟随系统，并会在系统偏好变化时实时同步；手动选择始终优先。
- 构建产物统一写入 `public/`，不再提交到 Git。
- 每次推送 `master` 都会由 GitHub Actions 重新构建、检查并发布。
- 私密文章可保存在同一仓库的加密档案中，通过隐蔽阅读页在浏览器本地解锁；不需要数据库。
...
私密文章的隐藏、重新加密、恢复和重新公开流程也记录在上述发布文档中。私密密码不会提交到 GitHub。
